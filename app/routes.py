...
def signup():
    if request.method == 'POST':
        ...
        if user_type == 'organization':
            cursor.execute(
                "INSERT INTO organization (organization_id, name, address, contact_num) VALUES (%s, %s, %s, %s)",
...
def login():
    if request.method == 'POST':
        ...
        conn = get_db_connection()
        cursor = cursor = conn.cursor(cursor_factory=psycopg2.extras.DictCursor)
...
def predict(student_id):
    ...
    cursor = conn.cursor()
    print('--------------------------\n',student_id)
    cursor.execute("SELECT COUNT(*) FROM latest_results WHERE stud_id = %s", (student_id,))
...
def doctors():
    ...
    conn = get_db_connection()
    cursor = conn.cursor(cursor_factory=psycopg2.extras.DictCursor)  # Changed for PostgreSQL compatibility
